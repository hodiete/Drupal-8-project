...
class CustomerEntityStorage extends SqlContentEntityStorage implements CustomerEntityStorageInterface {
  ...
  public function revisionIds(CustomerEntityInterface $entity) {
    return $this->database->query(
      'SELECT vid FROM {customer_entities_revision} WHERE id=:id ORDER BY vid',
      [':id' => $entity->id()]
    )->fetchCol();
  }
  ...
  public function userRevisionIds(AccountInterface $account) {
    return $this->database->query(
      'SELECT vid FROM {customer_entities_field_revision} WHERE uid = :uid ORDER BY vid',
      [':uid' => $account->id()]
    )->fetchCol();
  }
  ...
  public function countDefaultLanguageRevisions(CustomerEntityInterface $entity) {
    return $this->database->query('SELECT COUNT(*) FROM {customer_entities_field_revision} WHERE id = :id AND default_langcode = 1', [':id' => $entity->id()])
      ->fetchField();
  }
  ...
  public function clearRevisionsLanguage(LanguageInterface $language) {
    return $this->database->update('customer_entities_revision')
